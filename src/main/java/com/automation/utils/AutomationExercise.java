...
public class AutomationExercise extends BaseAutomationExcercise{
    ...
    public static int removeItemsFromCart(){
        //go to Cart page
        driver.findElement(By.xpath("//*[@id='header']/div/div/div/div[2]/div/ul/li[3]/a")).click();
        //CHeck if items are there on cart page or not
        if(driver.findElement(By.xpath("//*[@id='empty_cart']/p/b")).isDisplayed())
            existingNoOfItemsInCart = 0;
        else {
            existingNoOfItemsInCart = Integer.parseInt(driver.findElement(By.xpath("//td[@class='cart_quantity']/button")).getText());
            System.out.println("Before removing items from cart ->"+existingNoOfItemsInCart);
            //remove items from cart
            driver.findElement(By.xpath("//*[@id='product-1']/td[6]/a/i")).click();
            if(driver.findElement(By.xpath("//*[@id='empty_cart']/p/b")).getText().equals("Cart is empty!"))
                existingNoOfItemsInCart = 0;

        }
        return existingNoOfItemsInCart;
    }
    public static void validateItemQuantityInCart(int expectedNoOfItemInCart){
        Assert.assertEquals(existingNoOfItemsInCart, expectedNoOfItemInCart, "No of items do not match , test failed." );
